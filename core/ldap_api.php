...
////7777: LDAP Patch: añadimos fichero con funciones de obtener valores de conf de ldap
require_once(dirname(dirname(__FILE__)) . DIRECTORY_SEPARATOR .'LDAP_PATCH/lib/utils.php');
////7777//
 ...
 */
////7777: LDAP Patch: incluir parametro logon_to
////function ldap_connect_bind( $p_binddn = '', $p_password = '' ) {
function ldap_connect_bind( $p_binddn = '', $p_password = '', $p_logon_to = false ) {
////7777//
	...
	////7777: LDAP Patch: incluir parametro logon_to, y quito el parámetro ldap_port porque en esta versión de mantis no se usa
	...
	if (empty($p_logon_to)) {
		...
		$t_ldap_server = getCustomLDAPConfigField( 'ldap_server', $p_logon_to );
		////$t_ldap_port = getCustomLDAPConfigField( 'ldap_port', $p_logon_to );
	}
	////7777//
	...
	if ( $t_ds !== false && $t_ds > 0 ) {
		...
		if (empty($p_logon_to)) {
			...
			$t_protocol_version = getCustomLDAPConfigField( 'ldap_protocol_version', $p_logon_to );
		}
		////7777//
		...
		if (empty($p_logon_to)) {
			...
			$t_follow_referrals = ON == getCustomLDAPConfigField( 'ldap_follow_referrals', $p_logon_to );
		}
		////7777//
		...
		if( is_blank( $p_binddn ) && is_blank( $p_password ) ) {
			...
			if (empty($p_logon_to)) {
				...
				$p_binddn = getCustomLDAPConfigField( 'ldap_bind_dn', $p_logon_to );
				$p_password = getCustomLDAPConfigField( 'ldap_bind_passwd', $p_logon_to );
			}
			////7777//
		}
 ...
 * @return string
 */
////7777: LDAP Patch: incluimos parámetro logon_to
////function ldap_email_from_username( $p_username ) {
function ldap_email_from_username( $p_username, $p_logon_to = false ) {
////
	...
	$t_email = ldap_get_field_from_username( $p_username, 'mail', $p_logon_to );
	////7777//
...
function ldap_realname( $p_user_id, $p_logon_to = false ) {
////7777//
	...
	return ldap_realname_from_username( $t_username, $p_logon_to );
	////7777//
}
...
function ldap_realname_from_username( $p_username, $p_logon_to = false) {
////7777//
	if ( ldap_simulation_is_enabled() ) {
	...
	if (empty($p_logon_to)) {
		...
		$t_ldap_realname_field	= getCustomLDAPConfigField( 'ldap_realname_field', $p_logon_to );
	}
	////7777//

	////7777: LDAP Patch: incluir parametro logon_to
	////$t_realname = ldap_get_field_from_username( $p_username, $t_ldap_realname_field );
	$t_realname = ldap_get_field_from_username( $p_username, $t_ldap_realname_field, $p_logon_to );
	////7777//
...
function ldap_get_field_from_username( $p_username, $p_field, $p_logon_to = false ) {
////7777//
	...
	if (empty($p_logon_to)) {
		...
		$t_ldap_organization    = getCustomLDAPConfigField( 'ldap_organization', $p_logon_to );
		$t_ldap_root_dn         = getCustomLDAPConfigField( 'ldap_root_dn', $p_logon_to );
		$t_ldap_uid_field		= getCustomLDAPConfigField( 'ldap_uid_field', $p_logon_to );
	}
	////7777//
...
function ldap_has_group( $p_user_id, $p_group, $p_logon_to = false ) {
////7777//
	...
	if (empty($p_logon_to)) {
		...
		$t_ldap_uid_field = getCustomLDAPConfigField( 'ldap_uid_field', 'uid' );
	}
	////7777//
...
function ldap_authenticate( $p_user_id, $p_password, $p_logon_to = false ) {
////7777//
	...
	return ldap_authenticate_by_username( $t_username, $p_password, $p_logon_to );
	////7777//
}
...
function ldap_authenticate_by_username( $p_username, $p_password, $p_logon_to = false ) {
////7777//
	if ( ldap_simulation_is_enabled() ) {
		...
		////7777: LDAP Patch: incluir parametro logon_to
		//$t_ds = ldap_connect_bind();
		$t_ds = ldap_connect_bind('', '', $p_logon_to);
		////7777//
